...
int main()
{
	...
	while (true) {
		...
		_hsv[2] = magn_norm;
		Mat direction_mask(_hsv[0].size(), _hsv[0].type());
		threshold(_hsv[0], direction_mask, 90.f / 255.f, 180.f / 255.f, THRESH_TOZERO_INV); // 凡大于180度的都是0
		Mat vMasked = _hsv[0].setTo(0);
		_hsv[2].copyTo(vMasked, direction_mask);
		_hsv[2] = vMasked;
